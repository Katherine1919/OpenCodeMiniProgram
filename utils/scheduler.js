...
function generateSchedule(tasks, templates, date) {
  ...
  fixedTasks.forEach(task => {
    ...
    if (!canPlace(startMin, endMin, tmpl, usedSlots)) {
      ...
      usedSlots.forEach(slot => {
        const existingTask = fixedTasks.find(t => {
          const tStart = timeToMinutes(t.startTime);
          const tEnd = timeToMinutes(t.endTime);
          return tStart === slot.start && tEnd === slot.end;
        });
        if (existingTask) {
          conflicts.push({
            aTaskId: task.id,
            aTitle: task.title,
            aStart: task.startTime,
            aEnd: task.endTime,
            bTaskId: existingTask.id,
            bTitle: existingTask.title,
            bStart: existingTask.startTime,
            bEnd: existingTask.endTime,
            reason: 'time_overlap'
          });
        }
      });
  ...
  const conflictTaskIds = new Set(conflicts.flatMap(c => [c.aTaskId, c.bTaskId]));
  const outOfTemplateTaskIds = new Set(outOfTemplate.map(o => o.taskId));
